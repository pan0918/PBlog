  ...
  return (
    ...
    >
      <div className="rounded-2xl bg-white/80 dark:bg-slate-800/80 border border-white/40 dark:border-white/10 shadow-2xl p-3 flex items-center gap-3 cursor-grab active:cursor-grabbing">
        <div className="relative w-12 h-12 rounded-full overflow-hidden flex-shrink-0 animate-[spin_12s_linear_infinite]" style={{ animationPlayState: isPlaying ? 'running' : 'paused' }}>
          {currentSong.pic ? (
            <Image
              src={currentSong.pic}
              alt="cover"
        ...
        <div className="flex-1 min-w-0">
          ...
          <MusicProgressBar
            accentColor="#818cf8"
            trackColor="rgba(148,163,184,0.4)"
            containerClassName="flex items-center gap-1 mt-1"
            rangeClassName="flex-1 h-1 bg-slate-200 dark:bg-slate-700 rounded-full appearance-none outline-none cursor-pointer"
            timeClassName="text-[9px] text-slate-400 font-mono w-auto"
          />
        </div>
