...
import { requestPublicAccount } from '../../lib/public-auth/session-events';
      ...
      {expanded && (
        <>
            ...
            {data.session ? (
              <>
                ...
                {data.session.mustChangePassword ? (
                  <button type="button" onClick={() => requestPublicAccount()} className="w-full rounded-2xl border border-amber-200 bg-amber-50 px-3 py-3 text-left text-xs font-bold leading-5 text-amber-800 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-300">请点击右上角头像修改临时密码，再发布评论</button>
                ) : (
                  <CommentComposer onSubmit={(content) => data.submitComment(content)} />
                )}
              </>
            ) : (
              <button type="button" onClick={() => requestPublicAccount()} className="w-full rounded-2xl border border-dashed border-slate-300 bg-slate-50/70 px-4 py-5 text-left hover:border-indigo-300 hover:bg-indigo-50/60 focus-visible:outline-2 focus-visible:outline-indigo-500 dark:border-white/15 dark:bg-white/5 dark:hover:border-indigo-400/40 dark:hover:bg-indigo-500/10">
                <span className="block text-sm font-black text-slate-800 dark:text-slate-100">登录后参与讨论</span>
                <span className="mt-1 block text-xs leading-5 text-slate-500 dark:text-slate-400">点击右上角头像登录，可以评论、回复和点赞。</span>
              </button>
            )}
          </div>
          <div className={`comment-scroll min-h-0 flex-1 overflow-y-auto px-4 ${fullscreen ? 'pb-[calc(env(safe-area-inset-bottom)+1.5rem)]' : 'pb-4'}`}>
            <CommentList data={data} onNeedAuth={() => requestPublicAccount()} />
          </div>
        </>
      )}
